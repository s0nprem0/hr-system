...
export default async function () {
  ...
  try {
    ...
    if (admin) {
      const token = jwt.sign({ _id: admin._id, role: admin.role }, jwtKey, { expiresIn: '1h' });
      process.env.ADMIN_TOKEN = token;

      const refreshValue = crypto.randomBytes(48).toString('hex');
      const refresh = await RefreshToken.create({ token: refreshValue, user: admin._id, expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) });
