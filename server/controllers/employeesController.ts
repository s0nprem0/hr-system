...
const listEmployees = async (req: Request, res: Response) => {
  try {
    ...
    if (department && mongoose.isValidObjectId(department)) filter['profile.department'] = department;
...
const getEmployee = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const employee = await User.findById(id).select('-password').populate('profile.department', 'name');
...
const createEmployee = async (req: Request, res: Response) => {
  try {
    ...
    const created = await User.create({ name, email, password: hashed, role: role || 'employee', profile });
