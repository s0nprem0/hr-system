		...
		const fetchEmployee = async () => {
			...
			try {
				...
				if (r?.success) {
					...
					setDepartmentId(
						typeof dept === 'string' ? dept : dept?._id || undefined
					)
				} else {
					throw new Error(
						(r as { success: false; error?: { message?: string } }).error
		...
		try {
			if (isEdit && params.id) {
				...
				navTimeout.current = window.setTimeout(
					() => navigate(`/employees/${updated?._id || params.id}`),
					900
				)
			} else {
				...
				<form onSubmit={handleSubmit} className="">
					<FormCard>
						...
						<div>
							...
							/>
							{formErrors.name && (
								<div className="text-danger text-sm">{formErrors.name}</div>
							)}
						</div>
						...
						<div>
							...
							/>
							{formErrors.email && (
								<div className="text-danger text-sm">{formErrors.email}</div>
							)}
						</div>
						...
						<div>
							...
							/>
							{formErrors.password && (
								<div className="text-danger text-sm">{formErrors.password}</div>
							)}
						</div>
						...
						<div>
							<Select
								...
								onChange={(e: React.ChangeEvent<HTMLSelectElement>) =>
									setRole(e.target.value as 'admin' | 'hr' | 'employee')
								}
							>
